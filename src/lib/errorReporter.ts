...
class ErrorReporter {
  ...
  private async processQueue() {
    ...
    try {
      ...
      // If reporting fails, silently continue (don't re-queue to avoid cascading errors)
  ...
  private async sendError(error: ErrorReport) {
    // For local desktop app, just log errors to console instead of sending to remote endpoint
    try {
      console.group(`[ErrorReporter] ${error.level.toUpperCase()}: ${error.category || 'unknown'}`);
      console.error(error.message);
      if (error.stack) {
        console.error('Stack:', error.parsedStack || error.stack);
      }
      if (error.source) {
        console.error(`Source: ${error.source}:${error.lineno}:${error.colno}`);
      }
      console.error('URL:', error.url);
      console.error('Timestamp:', error.timestamp);
      console.groupEnd();
    } catch (err) {
      // Fail silently - don't throw to avoid cascading errors
...
const sendImmediateError = async (payload: ImmediatePayload): Promise<void> => {
  // For local desktop app, just log to console instead of sending to remote endpoint
  try {
    console.group(`[ErrorReporter] ${payload.level.toUpperCase()}: ${payload.category || 'unknown'}`);
